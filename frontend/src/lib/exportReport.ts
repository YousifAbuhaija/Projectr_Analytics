...
function labelFromScore(score: number): string {
  return score >= 70 ? "Strong Opportunity" : score >= 40 ? "Emerging Market" : "Saturated Market";
}
...
export async function exportToPDF(score: HousingPressureScore): Promise<void> {
  ...
  doc.setTextColor(150, 160, 180);
  doc.text(`${uni.city}, ${uni.state}  ·  ${uni.enrollment?.toLocaleString() ?? "—"} enrolled`, margin, 68);
  ...
  const enrollChange = latestEnroll && earliestEnroll
  ...
  autoTable(doc, {
    ...
    body: [
      ["Enrollment", latestEnroll?.toLocaleString() ?? "—", `${enrollChange} change`],
      ["Median Rent", latestRent ? `$${latestRent.toLocaleString()}` : "—", `${rentChange} change`],
      ["Permits (5yr)", totalPermits > 0 ? totalPermits.toLocaleString() : "—", "residential units"],
      ["Supply Pipeline", pipelinePct, "permits / enrollment"],
      ["County Housing Units", score.nearby_housing_units?.toLocaleString() ?? "—", "ACS estimate"],
      ["On-Campus Beds/Student",
        score.housing_capacity?.beds_per_student?.toFixed(2) ?? "—",
        score.housing_capacity?.dormitory_capacity?.toLocaleString() ?? "—" + " total beds"],
      ["Vacancy Rate", score.demographics?.vacancy_rate_pct != null
        ? `${score.demographics.vacancy_rate_pct.toFixed(1)}%` : "—", "renter market"],
      ["Median Gross Rent", score.demographics?.median_gross_rent
        ? `$${score.demographics.median_gross_rent.toLocaleString()}` : "—", "ACS"],
      ["Median Home Value", score.demographics?.median_home_value
        ? `$${score.demographics.median_home_value.toLocaleString()}` : "—", "ACS"],
      ["Median Household Income", score.demographics?.median_household_income
        ? `$${score.demographics.median_household_income.toLocaleString()}` : "—", "ACS"],
      ["Renter-Occupied %", score.demographics?.pct_renter_occupied != null
  ...
  if (score.institutional_strength) {
    ...
    autoTable(doc, {
      ...
      body: [
        ["Strength Score", ist.strength_score != null ? `${ist.strength_score.toFixed(0)}/100 (${ist.strength_label})` : "—"],
        ["Ownership", ist.ownership_label ?? "—"],
        ["Retention Rate", ist.retention_rate != null ? `${(ist.retention_rate * 100).toFixed(0)}%` : "—"],
        ["Admission Rate", ist.admission_rate != null ? `${(ist.admission_rate * 100).toFixed(0)}%` : "—"],
        ["Endowment / Student", ist.endowment_per_student
          ? `$${ist.endowment_per_student.toLocaleString()}` : "—"],
        ["Total Endowment", ist.endowment_end
          ? `$${(ist.endowment_end / 1_000_000).toFixed(0)}M` : "—"],
        ["Pell Grant Rate", ist.pell_grant_rate != null ? `${(ist.pell_grant_rate * 100).toFixed(0)}%` : "—"],
      ],
      headStyles: { fillColor: DARK, textColor: [255, 255, 255], fontSize: 8 },
      bodyStyles: { fontSize: 8, textColor: DARK },
      alternateRowStyles: { fillColor: [248, 248, 252] },
  ...
  if (score.existing_housing) {
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ["Residential / Houses", `${eh.residential_buildings.toLocaleString()} / ${eh.house_buildings.toLocaleString()}`],
        ["Multifamily Density", `${eh.apartment_density_per_km2.toFixed(1)} / km²`],
  ...
  // ── Occupancy Ordinance ──
  if (score.occupancy_ordinance && score.occupancy_ordinance.ordinance_type !== "none") {
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ["Max Unrelated Occupants", ord.max_unrelated_occupants != null ? `≤${ord.max_unrelated_occupants}` : "No cap"],
  ...
  if (score.enrollment_trend.length > 0) {
    autoTable(doc, {
      ...
      body: score.enrollment_trend.map((e) => [e.year, e.total_enrollment.toLocaleString()]),
      headStyles: { fillColor: DARK, textColor: [255, 255, 255], fontSize: 8 },
      bodyStyles: { fontSize: 8, textColor: DARK },
      alternateRowStyles: { fillColor: [248, 248, 252] },
      margin: { left: margin, right: margin },
  ...
  if (score.permit_history.length > 0) {
    autoTable(doc, {
      ...
      body: score.permit_history.map((p) => [p.year, p.permits.toLocaleString()]),
      headStyles: { fillColor: DARK, textColor: [255, 255, 255], fontSize: 8 },
      bodyStyles: { fontSize: 8, textColor: DARK },
      alternateRowStyles: { fillColor: [248, 248, 252] },
      margin: { left: margin, right: margin },
  ...
  if (score.rent_history.length > 0) {
    autoTable(doc, {
      ...
      body: score.rent_history.map((r) => [r.year, `$${r.median_rent.toLocaleString()}`]),
      headStyles: { fillColor: DARK, textColor: [255, 255, 255], fontSize: 8 },
      bodyStyles: { fontSize: 8, textColor: DARK },
      alternateRowStyles: { fillColor: [248, 248, 252] },
      margin: { left: margin, right: margin },
...
export async function exportToDocx(score: HousingPressureScore): Promise<void> {
  ...
  const children: (Paragraph | Table)[] = [
    ...
    new Paragraph({
      children: [new TextRun({ text: `${uni.city}, ${uni.state}  ·  ${uni.enrollment?.toLocaleString() ?? "—"} enrolled`, color: "888888", size: 20 })],
      spacing: { after: 80 },
    }),
    new Paragraph({
      children: [new TextRun({ text: `Generated by CampusLens  ·  ${score.scored_at ? new Date(score.scored_at).toLocaleDateString() : new Date().toLocaleDateString()}`, color: "AAAAAA", size: 16 })],
      spacing: { after: 360 },
    }),
    ...
    sectionTable([
      ...
      ["Enrollment Pressure", `${score.components.enrollment_pressure.toFixed(1)} / 100`],
    ...
    ...(score.gemini_summary ? [
      ...
      new Paragraph({ text: score.gemini_summary, spacing: { after: 200 }, style: "Normal" }),
    ] : []),
    ...
    sectionTable([
      ...
      ["Median Rent (latest)", latestRent ? `$${latestRent.toLocaleString()}` : "—"],
      ["Rent Change", rentChange],
      ["Permits Filed (5yr)", totalPermits > 0 ? totalPermits.toLocaleString() : "—"],
      ["Supply Pipeline", pipelinePct],
      ["County Housing Units", score.nearby_housing_units?.toLocaleString() ?? "—"],
      ["On-Campus Beds/Student", score.housing_capacity?.beds_per_student?.toFixed(2) ?? "—"],
      ["Total Dorm Capacity", score.housing_capacity?.dormitory_capacity?.toLocaleString() ?? "—"],
    ]),
    ...
    ...(dem ? [
      ...
      sectionTable([
        ["Median Household Income", dem.median_household_income ? `$${dem.median_household_income.toLocaleString()}` : "—"],
        ["Median Home Value", dem.median_home_value ? `$${dem.median_home_value.toLocaleString()}` : "—"],
        ["Median Gross Rent", dem.median_gross_rent ? `$${dem.median_gross_rent.toLocaleString()}` : "—"],
        ["Vacancy Rate", dem.vacancy_rate_pct != null ? `${dem.vacancy_rate_pct.toFixed(1)}%` : "—"],
        ["Renter-Occupied", dem.pct_renter_occupied != null ? `${dem.pct_renter_occupied.toFixed(1)}%` : "—"],
        ["Bachelors or Higher", dem.pct_bachelors_or_higher != null ? `${dem.pct_bachelors_or_higher.toFixed(1)}%` : "—"],
        ["Total Housing Units", dem.total_housing_units?.toLocaleString() ?? "—"],
      ]),
    ] : []),
    ...
    ...(ist ? [
      ...
      sectionTable([
        ...
        ["Retention Rate", ist.retention_rate != null ? `${(ist.retention_rate * 100).toFixed(0)}%` : "—"],
        ["Admission Rate", ist.admission_rate != null ? `${(ist.admission_rate * 100).toFixed(0)}%` : "—"],
        ["Endowment / Student", ist.endowment_per_student ? `$${ist.endowment_per_student.toLocaleString()}` : "—"],
        ["Total Endowment", ist.endowment_end ? `$${(ist.endowment_end / 1_000_000).toFixed(0)}M` : "—"],
        ["Pell Grant Rate", ist.pell_grant_rate != null ? `${(ist.pell_grant_rate * 100).toFixed(0)}%` : "—"],
      ]),
    ] : []),
    ...
    ...(eh ? [
      ...
      sectionTable([
        ...
        ["Apartment Buildings", eh.apartment_buildings.toLocaleString()],
        ["Dormitory Buildings", eh.dormitory_buildings.toLocaleString()],
        ["House / Residential Buildings", `${eh.house_buildings.toLocaleString()} / ${eh.residential_buildings.toLocaleString()}`],
        ["Multifamily Density", `${eh.apartment_density_per_km2.toFixed(1)} / km²`],
        ["Search Radius", `${eh.radius_miles} miles`],
      ]),
    ] : []),
    ...
    ...(score.disaster_risk ? [
      ...
      sectionTable([
        ...
        ["Weather Disasters", score.disaster_risk.weather_disasters.toString()],
        ["Window", `${score.disaster_risk.window_years} years`],
        ...(score.disaster_risk.most_recent_year ? [["Most Recent Year", score.disaster_risk.most_recent_year.toString()] as [string, string]] : []),
      ]),
    ] : []),
    ...
    ...(score.enrollment_trend.length > 0 ? [
      ...
      new Table({
        ...
        rows: [
          new TableRow({
            children: ["Year", "Enrollment"].map(h => new TableCell({
              children: [new Paragraph({ children: [new TextRun({ text: h, bold: true, size: 18 })] })],
            })),
          }),
          ...score.enrollment_trend.map(e => new TableRow({
            children: [e.year.toString(), e.total_enrollment.toLocaleString()].map(v => new TableCell({
              children: [new Paragraph({ children: [new TextRun({ text: v, size: 18 })] })],
            })),
          })),
        ],
    ...
    ...(score.permit_history.length > 0 ? [
      ...
      new Table({
        ...
        rows: [
          new TableRow({
            children: ["Year", "Permits"].map(h => new TableCell({
              children: [new Paragraph({ children: [new TextRun({ text: h, bold: true, size: 18 })] })],
            })),
          }),
          ...score.permit_history.map(p => new TableRow({
            children: [p.year.toString(), p.permits.toLocaleString()].map(v => new TableCell({
              children: [new Paragraph({ children: [new TextRun({ text: v, size: 18 })] })],
            })),
          })),
        ],
    ...
    ...(score.rent_history.length > 0 ? [
      ...
      new Table({
        ...
        rows: [
          new TableRow({
            children: ["Year", "Median Rent"].map(h => new TableCell({
              children: [new Paragraph({ children: [new TextRun({ text: h, bold: true, size: 18 })] })],
            })),
          }),
          ...score.rent_history.map(r => new TableRow({
            children: [r.year.toString(), `$${r.median_rent.toLocaleString()}`].map(v => new TableCell({
              children: [new Paragraph({ children: [new TextRun({ text: v, size: 18 })] })],
            })),
          })),
        ],
...
export async function exportComparisonToPDF(
  ...
  const [labelA, labelB] = resolveCompareLabels(uniA.name, uniA.city, uniB.name, uniB.city);
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(255, 255, 255);
  doc.text(`${uniA.name}  vs  ${uniB.name}`, margin, 50, { maxWidth: contentW });
  ...
  autoTable(doc, {
    ...
    body: [
      [
        ...
        scoreA.components.enrollment_pressure >= scoreB.components.enrollment_pressure ? labelA : labelB,
      ],
      [
        "Permit Gap",
        scoreA.components.permit_gap.toFixed(1),
        scoreB.components.permit_gap.toFixed(1),
        scoreA.components.permit_gap >= scoreB.components.permit_gap ? labelA : labelB,
      ],
      [
        "Rent Pressure",
        scoreA.components.rent_pressure.toFixed(1),
        scoreB.components.rent_pressure.toFixed(1),
        scoreA.components.rent_pressure >= scoreB.components.rent_pressure ? labelA : labelB,
      ],
      [
        "Overall Score",
  ...
  autoTable(doc, {
    ...
    body: [
      ["Enrollment", enrollA?.toLocaleString() ?? "—", enrollB?.toLocaleString() ?? "—"],
      ["Median Rent", rentA ? `$${rentA.toLocaleString()}` : "—", rentB ? `$${rentB.toLocaleString()}` : "—"],
      ["Permits (5yr)", permitsA > 0 ? permitsA.toLocaleString() : "—", permitsB > 0 ? permitsB.toLocaleString() : "—"],
      ["County Housing Units",
        scoreA.nearby_housing_units > 0 ? scoreA.nearby_housing_units.toLocaleString() : "—",
        scoreB.nearby_housing_units > 0 ? scoreB.nearby_housing_units.toLocaleString() : "—"],
      ["Beds/Student",
        scoreA.housing_capacity?.beds_per_student?.toFixed(2) ?? "—",
        scoreB.housing_capacity?.beds_per_student?.toFixed(2) ?? "—"],
      ["Vacancy Rate",
        scoreA.demographics?.vacancy_rate_pct != null ? `${scoreA.demographics.vacancy_rate_pct.toFixed(1)}%` : "—",
        scoreB.demographics?.vacancy_rate_pct != null ? `${scoreB.demographics.vacancy_rate_pct.toFixed(1)}%` : "—"],
      ["Median Gross Rent",
        scoreA.demographics?.median_gross_rent ? `$${scoreA.demographics.median_gross_rent.toLocaleString()}` : "—",
        scoreB.demographics?.median_gross_rent ? `$${scoreB.demographics.median_gross_rent.toLocaleString()}` : "—"],
      ["Renter-Occupied %",
        scoreA.demographics?.pct_renter_occupied != null ? `${scoreA.demographics.pct_renter_occupied.toFixed(1)}%` : "—",
        scoreB.demographics?.pct_renter_occupied != null ? `${scoreB.demographics.pct_renter_occupied.toFixed(1)}%` : "—"],
      ["Institutional Strength",
...
export async function exportComparisonToDocx(
  ...
  const uniA = scoreA.university;
  const uniB = scoreB.university;
  const [docxLabelA, docxLabelB] = resolveCompareLabels(uniA.name, uniA.city, uniB.name, uniB.city);
  ...
  function compRow(label: string, valA: string, valB: string) {
    return new TableRow({
      children: [label, valA, valB].map((text, i) => new TableCell({
        borders,
  ...
  const children = [
    new Paragraph({
      children: [new TextRun({ text: "Market Comparison Report", bold: true, size: 40 })],
      spacing: { after: 120 },
    }),
    new Paragraph({
      children: [new TextRun({ text: `${uniA.name}  vs  ${uniB.name}`, size: 24, color: "444444" })],
      spacing: { after: 80 },
    }),
    new Paragraph({
      children: [new TextRun({ text: `Generated by CampusLens  ·  ${new Date().toLocaleDateString()}`, size: 16, color: "AAAAAA" })],
      spacing: { after: 360 },
    }),

    heading("Overall Scores", HeadingLevel.HEADING_1),
    compTable([
      ["Overall Score", `${scoreA.score.toFixed(1)} / 100`, `${scoreB.score.toFixed(1)} / 100`],
      ["Classification", `${labelFromScore(scoreA.score)}`, `${labelFromScore(scoreB.score)}`],
      ["Enrollment Pressure", `${scoreA.components.enrollment_pressure.toFixed(1)}`, `${scoreB.components.enrollment_pressure.toFixed(1)}`],
      ["Permit Gap", `${scoreA.components.permit_gap.toFixed(1)}`, `${scoreB.components.permit_gap.toFixed(1)}`],
      ["Rent Pressure", `${scoreA.components.rent_pressure.toFixed(1)}`, `${scoreB.components.rent_pressure.toFixed(1)}`],
    ]),
    ...
    compTable([
      ...
      ["Median Gross Rent",
        ...
        scoreB.demographics?.median_gross_rent ? `$${scoreB.demographics.median_gross_rent.toLocaleString()}` : "—"],
      ["Median Home Value",
        scoreA.demographics?.median_home_value ? `$${scoreA.demographics.median_home_value.toLocaleString()}` : "—",
        scoreB.demographics?.median_home_value ? `$${scoreB.demographics.median_home_value.toLocaleString()}` : "—"],
    ]),

    heading("Institutional Strength", HeadingLevel.HEADING_2),
    compTable([
      ["Strength Score",
        scoreA.institutional_strength?.strength_score != null ? `${scoreA.institutional_strength.strength_score.toFixed(0)}/100 (${scoreA.institutional_strength.strength_label})` : "—",
        scoreB.institutional_strength?.strength_score != null ? `${scoreB.institutional_strength.strength_score.toFixed(0)}/100 (${scoreB.institutional_strength.strength_label})` : "—"],
      ["Ownership",
        scoreA.institutional_strength?.ownership_label ?? "—",
        scoreB.institutional_strength?.ownership_label ?? "—"],
      ["Retention Rate",
        scoreA.institutional_strength?.retention_rate != null ? `${(scoreA.institutional_strength.retention_rate * 100).toFixed(0)}%` : "—",
        scoreB.institutional_strength?.retention_rate != null ? `${(scoreB.institutional_strength.retention_rate * 100).toFixed(0)}%` : "—"],
      ["Admission Rate",
        scoreA.institutional_strength?.admission_rate != null ? `${(scoreA.institutional_strength.admission_rate * 100).toFixed(0)}%` : "—",
        scoreB.institutional_strength?.admission_rate != null ? `${(scoreB.institutional_strength.admission_rate * 100).toFixed(0)}%` : "—"],
      ["Endowment/Student",
        scoreA.institutional_strength?.endowment_per_student ? `$${scoreA.institutional_strength.endowment_per_student.toLocaleString()}` : "—",
        scoreB.institutional_strength?.endowment_per_student ? `$${scoreB.institutional_strength.endowment_per_student.toLocaleString()}` : "—"],
    ]),

    heading("Supply & Competition", HeadingLevel.HEADING_2),
    compTable([
      ["Housing Saturation", scoreA.existing_housing?.saturation_label ?? "—", scoreB.existing_housing?.saturation_label ?? "—"],
      ["Apartment Buildings",
        scoreA.existing_housing?.apartment_buildings?.toLocaleString() ?? "—",
        scoreB.existing_housing?.apartment_buildings?.toLocaleString() ?? "—"],
      ["Multifamily Density",
        scoreA.existing_housing ? `${scoreA.existing_housing.apartment_density_per_km2.toFixed(1)}/km²` : "—",
        scoreB.existing_housing ? `${scoreB.existing_housing.apartment_density_per_km2.toFixed(1)}/km²` : "—"],
      ["Weather Disasters",
        scoreA.disaster_risk?.weather_disasters?.toString() ?? "—",
        scoreB.disaster_risk?.weather_disasters?.toString() ?? "—"],
    ]),
  ...
  const doc = new Document({ sections: [{ children }] });
  const blob = await Packer.toBlob(doc);
  blobDownload(blob, `${slugify(uniA.name)}_vs_${slugify(uniB.name)}_campuslens.docx`);
}
